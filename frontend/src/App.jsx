...
import QrScanner from 'react-qr-scanner'; // Library Scanner Baru
...
const BuyModal = ({ item, user, userPickup, userDropOff, onClose, onSuccess }) => {
  ...
  const checkPromo = () => {
    if(!promoCode) return;
    setLoading(true);
    axios.post(`${config.API_BASE_URL}/check-promo`, { code: promoCode })
      .then(res => {
        if(res.data.valid) {
  ...
  const handleBuy = () => {
    ...
    axios.post(`${config.API_BASE_URL}/beli`, {
      idRute: item.id, 
        ...
        if (res.data.token && window.snap) {
            window.snap.pay(res.data.token, {
                onSuccess: function(result){
                    // Auto-check status ke backend (Solusi Localhost)
                    axios.post(`${config.API_BASE_URL}/check-status`, { orderId: result.order_id })
                        .then((res) => {
                            if (res.data.mintingError) {
                                alert("✅ Pembayaran Berhasil! Tapi Minting Blockchain Gagal: " + res.data.mintingError);
                            } else {
                                alert("✅ Pembayaran Berhasil! Tiket diterbitkan & Email dikirim.");
                            }
                            onSuccess(); // Pindah tab ke history
                            onClose();
                        })
                        .catch(() => {
                            // Fallback jika auto-check gagal
                            alert("Pembayaran sukses. Silakan cek status manual di tiket.");
                            onSuccess();
                            onClose();
                        });
                },
...
function Dashboard() {
  ...
  useEffect(() => {
      if (activeTab === 'admin') {
          axios.get(`${config.API_BASE_URL}/admin/stats`)
            .then(res => setStats(res.data))
            .catch(err => console.error("Gagal ambil stats:", err));

          axios.get(`${config.API_BASE_URL}/admin/logs`)
            .then(res => setAuditLogs(res.data))
            .catch(err => console.error("Gagal ambil logs:", err));
      }
  }, [activeTab]);

  const handleScan = (data) => {
      if (data) {
          setIsScanning(false); // Stop scanning sementara
          // data.text berisi Hash atau string dari QR
          const scannedHash = data.text;
          
          axios.post(`${config.API_BASE_URL}/verify-ticket`, { hash: scannedHash })
            .then(res => {
                if (res.data.valid) {
                    Swal.fire({
                        title: '✅ TIKET VALID!',
                        html: `
                            <div style="text-align:left;">
                                <p><b>Penumpang:</b> ${res.data.data.nama}</p>
                                <p><b>Rute:</b> ${res.data.data.rute}</p>
                                <p><b>Jadwal:</b> ${res.data.data.tanggal} ${res.data.data.jam}</p>
                                <p><b>Kursi:</b> ${res.data.data.seat}</p>
                                <p><b>Status:</b> ${res.data.data.status}</p>
                            </div>
                        `,
                        icon: 'success'
                    });
  ...
  useEffect(() => {
      axios.get(`${config.API_BASE_URL}/kota`)
  ...
  const fetchHistory = useCallback(() => {
    if (!user) return;
    axios.get(`${config.API_BASE_URL}/orders/${user.email}`)
  ...
  const fetchRute = () => {
    ...
    const url = `${config.API_BASE_URL}/rute?tanggal=${tanggal}&asal=${cariAsal}&tujuan=${cariTujuan}&lokasi=${cariLokasi}&turun=${cariTurun}`;
      ...
      <div className="main-container">
                                ...
                                {order.status === 'PENDING' ? (
                                    <div style={{textAlign:'center'}}>
                                        ...
                                        <div style={{display:'flex', flexDirection:'column', gap:'10px'}}>
                                            ...
                                            <button 
                                                onClick={() => {
                                                    axios.post('http://localhost:3000/check-status', { orderId: order.orderId_Midtrans })
                                                        .then(res => {
                                                            if(res.data.orderStatus === 'LUNAS' || res.data.orderStatus === 'MINTED') {
                                                                if (res.data.mintingError) {
                                                                    alert("✅ LUNAS! Tapi Minting Gagal: " + res.data.mintingError);
                                                                } else {
                                                                    alert("✅ Status Diperbarui! Tiket Lunas.");
                                                                }
                                                                fetchHistory();
                                                            } else {
                                                                alert("ℹ️ Status masih " + res.data.orderStatus + ". Jika sudah bayar, tunggu sebentar lalu cek lagi.");
                                                            }
                                                        })
                                                        .catch(err => alert("Gagal cek status: " + err.message));
                                                }} 
                                                style={{background:'#fff', color:'#1E3A8A', border:'1px solid #1E3A8A', padding:'8px 20px', borderRadius:'8px', fontWeight:'bold', cursor:'pointer'}}
                                            >
                                                🔄 Cek Status
                                            </button>
                                        </div>
                                    ...
                                    <>
                                        ...
                                        {order.hash && order.hash.length > 20 ? (
                                            <a href={`https://sepolia.etherscan.io/tx/${order.hash}`} target="_blank" rel="noreferrer" style={{fontSize:'0.6rem', color:'#2563EB', margin:'10px 0', wordBreak:'break-all', fontFamily:'monospace', textDecoration:'underline', cursor:'pointer'}} title="Klik untuk Cek di Etherscan">
                                                HASH: {order.hash.substring(0, 10)}...{order.hash.substring(order.hash.length - 6)} 🔗
                                            </a>
                                        ) : (
        ...
        {activeTab === 'admin' && (
          <div className="admin-card">
            ...
            {/* --- SCANNER SECTION --- */}
            <div style={{background:'white', padding:'20px', borderRadius:'16px', marginBottom:'30px', boxShadow:'0 10px 30px rgba(0,0,0,0.1)', textAlign:'center'}}>
                <h3 style={{color:'#1E3A8A', marginTop:0}}>🕵️ Validator Tiket</h3>
                <p style={{color:'#64748B'}}>Scan QR Code penumpang untuk memverifikasi keaslian tiket via Blockchain.</p>
                
                {!isScanning ? (
                    <button onClick={() => setIsScanning(true)} style={{padding:'15px 30px', background:'#1E3A8A', color:'white', border:'none', borderRadius:'10px', fontSize:'1.1rem', cursor:'pointer', display:'inline-flex', alignItems:'center', gap:'10px'}}>
                        📷 Mulai Scan Kamera
                    </button>
                ) : (
                    <div style={{maxWidth:'400px', margin:'0 auto', border:'5px solid #1E3A8A', borderRadius:'10px', overflow:'hidden'}}>
                        <QrScanner
                            delay={300}
                            onError={handleError}
                            onScan={handleScan}
                        ...
                        />
                        <button onClick={() => setIsScanning(false)} style={{marginTop:'10px', padding:'10px', background:'#EF4444', color:'white', border:'none', borderRadius:'5px', cursor:'pointer', width:'100%'}}>Tutup Kamera</button>
                    </div>
                )}
            </div>
            ...
            <div style={{display:'grid', gridTemplateColumns:'1fr 1fr', gap:'20px', marginBottom:'30px'}}>
                <div style={{background:'linear-gradient(135deg, #3B82F6, #1E3A8A)', padding:'25px', borderRadius:'16px', color:'white'}}>
                    <div style={{fontSize:'0.9rem', opacity:0.8}}>Total Tiket Terjual</div>
                    <div style={{fontSize:'2.5rem', fontWeight:'800'}}>{stats.totalTiket}</div>
                    <div style={{fontSize:'0.8rem', marginTop:'5px'}}>🎫 Tiket Lunas/Minted</div>
                </div>
                <div style={{background:'linear-gradient(135deg, #10B981, #047857)', padding:'25px', borderRadius:'16px', color:'white'}}>
                    <div style={{fontSize:'0.9rem', opacity:0.8}}>Total Pendapatan</div>
                    <div style={{fontSize:'2.5rem', fontWeight:'800'}}>{formatRupiah(stats.totalPendapatan)}</div>
                    <div style={{fontSize:'0.8rem', marginTop:'5px'}}>💰 Revenue Kotor</div>
                </div>
            </div>
            ...
            {/* --- AUDIT LOGS SECTION --- */}
            <div style={{background:'white', padding:'25px', borderRadius:'16px', marginTop:'30px', boxShadow:'0 5px 20px rgba(0,0,0,0.05)'}}>
                <h4 style={{marginTop:0, marginBottom:'20px', color:'#1E3A8A', borderBottom:'2px solid #F1F5F9', paddingBottom:'10px'}}>🛡️ Audit Logs (Keamanan)</h4>
                        ...
                        <tbody>
                            {auditLogs.map((log, i) => (
                                <tr key={i} style={{borderBottom:'1px solid #f1f5f9'}}>
                                    ...
                                    <td style={{padding:'12px', fontWeight:'bold', color: log.action.includes('FAILED') || log.action.includes('INVALID') ? '#EF4444' : '#10B981'}}>
                                        {log.action}
                                    </td>
                                    <td style={{padding:'12px'}}>
                                        <div>{log.email || 'Anon'}</div>
                                        <div style={{fontSize:'0.7rem', color:'#94a3b8'}}>{log.ip}</div>
                                    </td>
                                    <td style={{padding:'12px'}}>{log.role}</td>
                                    <td style={{padding:'12px', maxWidth:'300px', overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap'}} title={log.details}>
                                        {log.details}
                                    </td>
                                </tr>
                            ))}
                            {auditLogs.length === 0 && (
                                <tr>
                                    <td colSpan="5" style={{padding:'20px', textAlign:'center', color:'#94a3b8'}}>Belum ada aktivitas tercatat.</td>
                                </tr>
                            )}
                        </tbody>
                    </table>
